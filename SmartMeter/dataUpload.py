#!/usr/bin/env python

import requests
import glob
import os
import time
from ConfigurationZ import DATA_BASE_PATH, THRESHOLD_TIME, DEVICE_ID
import datetime
import shutil
from Utilities import list_subdirectories
import datetime            

try:
    folder_list = list_subdirectories(DATA_BASE_PATH)
    print folder_list
        
    #files = glob.glob(DATA_BASE_PATH+list_dir[0]+"/*.csv")
    #print files

    # code to rename folders in month_day format
    #for folder in list_dir:
	#print folder
	#temp = folder.split("_")
	#print temp
	#print DATA_BASE_PATH + folder	
	#print DATA_BASE_PATH+temp[1]+"_"+temp[0]
	#os.rename(DATA_BASE_PATH+folder, DATA_BASE_PATH+temp[1]+"_"+temp[0])
    
    for folder in folder_list:
	print "In folder: " + DATA_BASE_PATH+folder + "...\n"		
	files = glob.glob(DATA_BASE_PATH + folder + "/*.csv")
	print "Files found: \n"
	#print files
	files.sort(key = lambda x : os.path.getmtime(x))
	#print files	
	#files = files.sort()
	#print DATA_BASE_PATH+folder+"/"+files[0]		
	#t = time.ctime(os.path.getctime(DATA_BASE_PATH+folder+"/"+files[0]))
	print datetime.datetime.fromtimestamp(t)	
	for f in files:
		print f + " " + time.ctime(os.path.getctime(f))
	


	#for d in list_dir:
        
     #   DATA_PATH =DATA_BASE_PATH+d+"/"
      #  print "Entering: "+DATA_PATH
       # list_of_files=glob.glob(DATA_PATH+str("/*.csv"))
        #for f in list_of_files:
            #try:
                #print "Entering: "+f            
                #if int(time.time())-int(os.stat(f).st_mtime)>THRESHOLD_TIME:
                
                    #head, tail = os.path.split(f)
                    #print "yo"          
                    #f1 = "D"+str(DEVICE_ID)+"_C"+tail
                    
                    #os.rename(f,DATA_PATH+f1)  
                              
                    #files = {'Data': open(DATA_PATH+f1, 'rb')}
                    #print "upload "+DATA_PATH+f1
                    #r = requests.post(url='http://sensoract.iiitd.edu.in:9006',files=files)
                    
                    #if r.status_code==200:
                    #    print "Upload Success. \n Now removing file: "+f1
                    #    os.remove(DATA_PATH+f1)
                    #    
                    #    lo=open(DATA_BASE_PATH+"LOG_UPLOAD.txt","a")
                    #    lo.write(str(time.time())+"  Removed: "+DATA_PATH+f1+"upload success \n")
                    #    lo.close()
                    #else:
                    #    os.rename(DATA_PATH+f1,f)                        
                        
            #except Exception as e:
                #lo=open(DATA_BASE_PATH+"LOG_UPLOAD.txt","a")
                #lo.write(str(time.time())+"  "+"filename: "+f +"  Error: "+e.__str__()+"\n")
                #lo.close()
		#print e.__str__()    
                
except Exception as e:
    lo=open(DATA_BASE_PATH+"LOG_UPLOAD.txt","a")
    lo.write(str(time.time())+"  "+e.__str__()+"\n")
    lo.close()
